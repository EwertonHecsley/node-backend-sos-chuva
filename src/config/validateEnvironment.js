...
const validateEnvironment = () => {
  ...
  if (missingEnvVars.length > 0) {
    Logger.error(
      `Falha critica: Variáveis ausentes -> ${missingEnvVars.join(', ')}`,
    );
    process.exit(1);
  }
  ...
  if (!Object.values(Environment).includes(process.env.NODE_ENV)) {
    Logger.error(
      `Falha critica: Valor inválido para NODE_ENV: ${process.env.NODE_ENV}. Deve ser 'production' ou 'development'.`,
    );
    process.exit(1);
  }
